...
class SaleExtendedWizard(orm.TransientModel):
    ...
    def change_pricelist_products(self, cr, uid, ids, context):
        ...
        sale_order_pool = self.pool['sale.order']
        ...
        if sale_obj.pricelist_id.id == pricelist_id.id:
            raise orm.except_orm(_('Warning'), _('The Pricelist is already applied to the sales order!'))

        if sale_obj['state'] in ('draft', 'sent'):
            sale_order_pool.write(cr, uid, context['active_id'], {'pricelist_id': pricelist_id.id})
            for line in sale_obj.order_line:
                vals = order_line_obj.product_id_change(cr, uid, line.id, pricelist_id.id, line.product_id.id,
                                                        qty=line.product_uom_qty, uom=line.product_uom.id,
                                                        uos=line.product_uos.id, partner_id=partner_id,
                                                        date_order=date_order)
                if vals.get('value', False):
                    if 'price_unit' in vals['value'].keys():
                        order_line_obj.write(cr, uid, line.id, {'price_unit': vals['value']['price_unit']},
                                             context=context)
        else:
            raise orm.except_orm(_('Warning'),
                                 _('PriceList cannot be changed! Make sure the Sales Order is in "Quotation" state!'))
        return {}
