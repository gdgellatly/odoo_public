...
class AddDepositItems(orm.TransientModel):
    ...
    def default_get(self, cr, uid, field, context=None):
        deposit_ticket_obj = self.pool['deposit.ticket']
        aml_obj = self.pool['account.move.line']
        move_obj = self.pool['account.move']
        result = []
        res = super(AddDepositItems, self).default_get(
            cr, uid, field, context=context)
        for deposit in deposit_ticket_obj.browse(
                cr, uid, context.get('active_ids', []), context=context):
            # Filter all the move lines which are:
            # 1. Account.move.lines that are a member of the Deposit 
            #    From GL Account and are Debits
            # 2. State of the move_id == Posted
            # 3. The Deposit Ticket # value is blank (null) not assigned
            # 4. The account move line is not a member of another Draft/To 
            #    Be Review (this is the list (one2many) of
            #    debit transactions displayed on the selected Account 
            #    (Undeposited Funds Acct) including this account.
            move_ids = move_obj.search(cr, uid, [('state', '=', 'posted')], 
                                       context=context)
            line_ids = aml_obj.search(
                cr, uid, 
                [('account_id', '=', deposit.deposit_from_account_id.id),
                 ...
                 ('deposit_id', '=', False)],
                context=context)
            for line in aml_obj.browse(cr, uid, line_ids, context=context):
        ...
        if 'deposit_items_line_ids' in field:
    ...
    def add_deposit_lines(self, cr, uid, ids, context=None):
        ...
        deposit_ticket_line_obj = self.pool['deposit.ticket.line']
        deposit_ticket_item = self.browse(cr, uid, ids[0], context=context)
...
class deposit_items_line(orm.orm_memory):
