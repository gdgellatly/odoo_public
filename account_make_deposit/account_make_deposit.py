...
class DepositTicket(orm.Model):
    ...
    def check_group(self, cr, uid, ids, context=None):
        ...
        group_verifier_id = model_data_obj._get_id(
            cr, uid, 'account_make_deposit', 'group_make_deposits_verifier')
            ...
            if group_verifier_id:
                ...
                if deposit.state != 'draft' and uid not in group_user_ids:
                    raise orm.except_orm(
                        _('User Error !'),
                        _("Only a member of '%s' group may delete/edit "
                          "deposit tickets when not in draft "
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(DepositTicket, self).unlink(cr, uid, ids, context=context)

    def write(self, cr, uid, ids, vals, context=None):
        self.check_group(cr, uid, ids, context)
        return super(DepositTicket, self).write(cr, uid, ids, vals, 
                                                context=context)
    ...
    def action_process(self, cr, uid, ids, context=None):
        ...
        3.The account.move.lines are updated and written with the 
          'Deposit Ticket #'
    ...
    def remove_all(self, cr, uid, ids, context=None):
        ...
        account moves, updating account.move.lines, resetting preparer and 
        verifier and verified date fields.
        ...
        for deposit in self.browse(cr, uid, ids, context=context):
            ...
            if deposit.move_id:
                move_obj.button_cancel(cr, uid, [deposit.move_id.id], 
                                       context=context)
                move_obj.unlink(cr, uid, [deposit.move_id.id], context=context)
                vals['draft_assigned'] = True
        aml_obj.write(cr, uid, move_line_ids, vals, context=context)
        return True
    ...
    def _get_period(self, cr, uid, context=None):
        periods = self.pool['account.period'].find(cr, uid)
    ...
    def _get_totals(self, cr, uid, ids, name, args, context=None):
        ...
        for deposit in self.browse(cr, uid, ids, context=context):
            res[deposit.id] = {
                'amount': sum([l.amount for l in deposit.ticket_line_ids]),
                'count_total': len(deposit.ticket_line_ids)}
        return res

    _columns = {
        'memo': fields.char(
            'Memo', size=64, states={'done': [('readonly', True)]}, 
        ...
        'period_id': fields.many2one(
            'account.period', 'Force Period', required=True,
            states={'done': [('readonly', True)]},
        ...
        'prepared_by_user_id': fields.many2one(
            'res.users', 'Prepared By', states={'done': [('readonly', True)]},
        ...
        'deposit_bag_no': fields.char(
            'Deposit Bag No', size=64, states={'done': [('readonly', True)]}, 
            help="Deposit Bag number for courier transit."),
        'bank_tracking_no': fields.char(
            'Deposit Tracking No', size=64,
            help="This field is used to hold a tracking number provided by "
                 "the bank/financial institution often used in Remote Deposit "
                 "Capture on a deposit receipt. "
        ...
        'name': fields.related(
            ...
            relation='account.move', string='Deposit Ticket #', 
            help="Each deposit will have a unique sequence ID. "
                 "System generated."),
        'ticket_line_ids': fields.one2many(
            'deposit.ticket.line', 'deposit_id', 'Deposit Ticket Line', 
            states={'done': [('readonly', True)]}),
        'amount': fields.function(
            _get_totals, multi='dep_totals', string='Amount', 
            digits_compute=dp('Account'), type='float', 
            help="Calculates the Total of All Deposit Lines – "
                 "This is the Total Amount of Deposit."),
        'count_total': fields.function(
            _get_totals, multi='dep_totals', type='integer', 
            string='Total Items', help="Counts the total # of line items in "
                                       "the deposit ticket."),
    ...
    def add_deposit_items(self, cr, uid, ids, context=None):
        """Display the wizard to allow the 'Deposit Preparer' to 
...
class DepositTicketLine(orm.Model):
    ...
    _columns = {
        ...
        'deposit_id': fields.many2one(
            'deposit.ticket', 'Deposit Ticket', required=True,
            ondelete='cascade'),
        'company_id': fields.related(
            'deposit_id', 'company_id', type='many2one',
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not vals.get('move_line_id', False):
            raise orm.except_orm(
                _('Processing Error'),
                _('You cannot add any new deposit ticket line '
                  'manually as of this revision!'
                  ' Please use the button "Add Deposit Items" to '
                  'add deposit ticket line!'))
        return super(DepositTicketLine, self).create(
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        aml_obj.write(cr, uid, move_line_ids, {'draft_assigned': False},
                      context=context)
        return super(DepositTicketLine, self).unlink(cr, uid, ids,
                                                     context=context)
