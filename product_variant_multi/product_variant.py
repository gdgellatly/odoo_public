...
class ProductVariantDimensionValue(orm.Model):
    ...
    _columns = {
        ...
        'product_ids': fields.many2many(
                            ...
                            'dimension_id', 'product_id', 'Variant',
                            readonly=True),
...
class ProductTemplate(orm.Model):
    ...
    def button_generate_variants(self, cr, uid, ids, context=None):
        ...
        def temp_insert_and_update():
            ...
            to_insert_val_args = '('+ ','.join(['%s' for x in range(len(vals[0]))]) + ')'
...
'''CREATE TEMP table ''' + temp_product_table + ''' ON COMMIT DROP AS
  ...
  DELETE FROM ''' + temp_product_table
        ...
        def generator(func):
            ...
            res = func(cr, uid, products, vals, context=context)
            t2 = time.time()
        ...
        for product_temp in self.browse(cr, uid, ids, context=context):
            ...
            created_product_ids = []
            if temp_val_list:
                list_of_variants = self._create_variant_list(cr, uid, ids,
                                                             temp_val_list,
                                                             context=context)
